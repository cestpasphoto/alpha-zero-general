        ...
        if self.nb_threads == 1:
            for _ in trange(self.args.numEps, desc="Self Play", ncols=120):
                ...
                if len(iterationTrainExamples) == self.args.maxlenOfQueue:
                    log.warning('saturation of elements in iterationTrainExamples, '
                                'think about decreasing numEps or increasing maxlenOfQueue'
                                )
                    break
        else:
            ...
            for lock in locks:
                lock.acquire()
            threads_list = [
                    Thread(target=self.executeEpisodes_batch,
        ...
        for i in range(1, self.args.numIters + 1):
            ...
            if not self.skipFirstSelfPlay or i > 1:
                ...
                if self.args.dirichletAlpha > 0 and not (1/1.5 < self.args.dirichletAlpha / (10/avg_valid_moves) < 1.5):
                    print(f'There are about {avg_valid_moves:.1f} valid moves per state, '
                          'so I advise to set dirichlet to {10/avg_valid_moves:.1f} instead')
            ...
            def p0_ran(x, _n):
                probs = nmcts.getActionProb(x, temp=1., force_full_search=True)[0]
            ...
            def p1_ran(x, _n):
                probs = pmcts.getActionProb(x, temp=1., force_full_search=True)[0]
                return np.random.choice(len(probs), p=probs)
            ...
            arena = Arena(p0_ran, p1_ran, self.game)
