...
class GenericNNetWrapper(NeuralNet):
    ...
    def loss_v(self, targets_V, targets_Q, outputs):
        ...
        return torch.sum((targets - outputs) ** 2) / (targets_V.size()[0] * targets_V.size()[-1])
        # Normalize by batch size * nb of players

    def save_checkpoint(self, folder='checkpoint', filename='checkpoint.pth.tar', additional_keys=None):
    ...
    def load_network(self, checkpoint, strict=False):
                ...
                if name in target_state:
                    ...
                    if target_params.shape == params.shape:
                        ...
                        if len(target_params.shape) == 1:
                            ...
                            min_size_0 = min(target_params.shape[0], params.shape[0])
                            min_size_1 = min(target_params.shape[1], params.shape[1])
                            target_params[:min_size_0, :min_size_1] = params[:min_size_0, :min_size_1]
                        elif len(target_params.shape) == 3:
                            min_size_0 = min(target_params.shape[0], params.shape[0])
                            min_size_1 = min(target_params.shape[1], params.shape[1])
                            min_size_2 = min(target_params.shape[2], params.shape[2])
                            target_params[:min_size_0, :min_size_1, :min_size_2] = params[:min_size_0, :min_size_1, :min_size_2]
                        elif len(target_params.shape) == 4:
                            min_size_0 = min(target_params.shape[0], params.shape[0])
                            min_size_1 = min(target_params.shape[1], params.shape[1])
                            min_size_2 = min(target_params.shape[2], params.shape[2])
                            min_size_3 = min(target_params.shape[3], params.shape[3])
                            target_params[:min_size_0, :min_size_1, :min_size_2, :min_size_3
                                          ] = params[:min_size_0, :min_size_1, :min_size_2, :min_size_3]
                        else:
                        ...
                        print(
                            f'{name}: couldnt match loaded {params.shape} and target {target_params.shape},'
        ...
        if strict and (checkpoint['full_model'].version != self.args['nn_version']):
            print('Checkpoint includes NN version',
                  checkpoint['full_model'].version,
                  ', but you ask version',
                  self.args['nn_version'],
                  ' so not loading it and initiate knowledge transfer')
